...
export default function MenuItemList({
  ...
  const handleDelete = async (id: number) => {
    ...
    if (cartitemError) {
      alert(
        "Failed to delete related cartitems: " +
          (cartitemError.message || JSON.stringify(cartitemError))
      );
    ...
    if (menuitemError) {
      alert(
        "Delete failed: " +
          (menuitemError.message || JSON.stringify(menuitemError))
      );
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {filteredItems.map((item) => (
          <div
            key={String(item.menuitem_id)}
            className="relative bg-gray-100 rounded-xl shadow p-3 flex flex-col h-full"
          >
            <button
      ...
      {modalItem && (
        <div
          ...
          style={{
            background: modalItem ? "rgba(0,0,0,0.6)" : "rgba(0,0,0,0)",
          }}
          aria-hidden={!modalItem}
        >
          <div
            className={`bg-white rounded-lg shadow-lg max-w-lg w-full p-6 relative transform transition-all duration-300 ${
              modalItem ? "scale-100 opacity-100" : "scale-95 opacity-0"
            }`}
